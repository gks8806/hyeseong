...
/**
 * REST API 서비스(댓글서비스)==REST FULL 서비스 만들기
 * @author hu-405
 *
 */
@RestController
...
public class ReplyController {
	...
	public ResponseEntity<String> deleteReply(@PathVariable("rno") Integer rno){//왜@RequestBody했냐면 JSON데이터값을 보내기위함이지.
		ResponseEntity<String> entity = null;
		try {
			replyService.deleteReply(rno);
			entity = new ResponseEntity<String>("SUCCESS", HttpStatus.OK);//성공했으면 초기값 SUCCESS설정
			
		} catch (Exception e) {//실패했으면 실행되는 구문
			e.printStackTrace();
			entity = new ResponseEntity<String>(e.getMessage(), HttpStatus.BAD_REQUEST);
		}
		return entity; //JSON데이터를 리턴(jsp 페이지로)
	}
	/**
	 * 댓글 수정 서비스
	 */
	@RequestMapping(value="/update/{rno}", method= {RequestMethod.PUT,RequestMethod.PATCH})
	public ResponseEntity<String> updateReply(@PathVariable("rno") Integer rno, @RequestBody ReplyVO replyVO){//왜@RequestBody했냐면 JSON데이터값을 보내기위함이지.
		ResponseEntity<String> entity = null;
		try {
			replyVO.setRno(rno);
	...
	public ResponseEntity<String> insertReply(@RequestBody ReplyVO replyVO){//왜@RequestBody했냐면 JSON데이터값을 보내기위함이지.
		ResponseEntity<String> entity = null;
		try {
			replyService.insertReply(replyVO);//여기까지 성공하면 끝
			entity = new ResponseEntity<String>("SUCCESS", HttpStatus.OK);//성공했으면 초기값 SUCCESS설정
