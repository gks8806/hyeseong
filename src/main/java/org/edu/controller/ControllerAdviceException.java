...
public class ControllerAdviceException {
	...
	public ModelAndView errorModelAndView(Exception ex, HttpServletRequest request) {
		...
		//세션으로 저장해서 jsp페이지에 prevPage 변수로 보내는 작업(아래코드)
		request.getSession().setAttribute("prevPage", referer);//세t
		//세션변수의 출현배경(아래)
		//브라우저는 stateless(무상태보전없음)기반이기 때문에...
		//링크이동시 이전페이지의 저장값을 이동한 페이지에서 ㅅ사라지기 때문에 stateless
